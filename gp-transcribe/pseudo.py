...
def mask(text: str, strict: bool = False):
    ...
    for e in ents:
        ...
        # A gliner "plate" span with NO digit is a false positive (it tags generic words like
        # "kodas"/"pažymėjimas" as a plate). Real plates carry digits.
        if typ in ("license plate", "vehicle registration number") and not any(ch.isdigit() for ch in t):
            continue
