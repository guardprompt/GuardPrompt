...
function fillComposeBody(payload) {
  ...
  // Collect EDITABLE candidates (the read message body is role=document, so excluded).
  const raw = document.querySelectorAll('[role="textbox"],[contenteditable="true"],[contenteditable=""],textarea');
  const cands = [];
  for (const el of raw) {
    let r; try { r = el.getBoundingClientRect(); } catch (e) { continue; }
    if (!r || r.width < 80 || r.height < 30) continue;
    try { const cs = getComputedStyle(el); if (cs && (cs.display === "none" || cs.visibility === "hidden")) continue; } catch (e) {}
    const editable = /^textarea$/i.test(el.tagName) || el.isContentEditable || el.getAttribute("contenteditable") === "true";
    if (!editable) continue;
    const lab = ((el.getAttribute("aria-label") || "") + " " + (el.getAttribute("placeholder") || "")).toLowerCase();
    cands.push({ el, area: r.width * r.height, isBody: /message body|laiško|laisko|тела|текст сообщения|body/.test(lab) });
  }
  if (!cands.length) return { ok: false, reason: "nerastas laiško kūno laukas" };
  // Prefer the element explicitly labelled as the message body (Outlook: aria-label
  // "Message body"); otherwise the largest editable. This avoids filling a search box /
  // the wrong editor.
  cands.sort((a, b) => (b.isBody - a.isBody) || (b.area - a.area));
  const best = cands[0].el;
  try { best.focus(); best.scrollIntoView({ block: "center" }); } catch (e) {}

  // TEXTAREA: native value + input event.
  if (/^textarea$/i.test(best.tagName)) {
    best.value = t + "\n\n" + (best.value || "");
  ...
  // CONTENTEDITABLE (Outlook = React): DOM manipulation alone does NOT update React's model,
  // so Send reads an empty body while the text sits orphaned. Must go through the real input
  // pipeline. Put the caret at the very start, then insert via beforeinput/execCommand so
  // React sees genuine InputEvents. insertParagraph between lines keeps line breaks.
  try {
    const sel = window.getSelection();
    ...
    for (let i = 0; i < lines.length; i++) {
      if (i > 0) {
        try { best.dispatchEvent(new InputEvent("beforeinput", { inputType: "insertParagraph", bubbles: true, cancelable: true })); } catch (e) {}
        document.execCommand("insertParagraph");
      }
      if (lines[i]) {
        try { best.dispatchEvent(new InputEvent("beforeinput", { inputType: "insertText", data: lines[i], bubbles: true, cancelable: true })); } catch (e) {}
  ...
  // Last-resort fallback: DOM insert (works on non-React editors).
  try {
    ...
    best.dispatchEvent(new Event("input", { bubbles: true }));
    return { ok: true, target: "contenteditable-fallback" };
  } catch (e) { return { ok: false, reason: "nepavyko įrašyti į kūną" }; }
}
...
function buildAgentPrompt(goal, notes, ctx) {
  ...
  return (
    ...
    '{"op":"search_m365","query":"raktažodžiai/vardas/tema"} — IEŠKOTI po VISĄ vartotojo Microsoft 365 (paštas, failai, SharePoint, Teams), NE tik šiame puslapyje. Naudok, kai reikia rasti laišką/failą/žmogų, kurio NĖRA ekrane („surask laišką nuo X“, „ar turiu laiškų iš Y“, „failai apie Z“). Rezultatą gausi pastabose; po to gali op=click atidaryti rastą įrašą pagal temą ir op=draft_reply atsakyti.\n' +
    ...
    "- MICROSOFT 365 (Outlook/SharePoint/Teams/Office): jei reikia informacijos, kurios NĖRA šiame puslapyje (laiškas nuo ko nors, senas laiškas, failas, „ar turiu…“) — pirmiausia op=search_m365. SUDĖTINIAM prašymui („surask X laišką IR atsakyk“) planuok kelis žingsnius: (1) op=search_m365 «X»; (2) op=click atidaryk rastą laišką pagal temą; (3) op=draft_reply. NEATSAKINĖK „nėra“ jei dar nepaieškojai su search_m365. Atsakymui į laišką VISADA op=draft_reply (jis atidaro Reply, parašo, įrašo, sustoja — žmogus siunčia).\n" +
...
async function runAgent(goal, model) {
  ...
  try {
    ...
    // Microsoft 365 → do NOT inject a (stale) OWUI skill: the system prompt already carries
    // the full M365 guidance (search_m365 + draft_reply + compound planning), and an old
    // outlook365/teams/sharepoint skill would override it and hide search_m365.
    const onM365 = /outlook\.(office|office365|live)\.com|outlook\.com|teams\.(microsoft|live|cloud)|[\w-]+\.sharepoint\.com|onedrive\.live|\.cloud\.microsoft|(^| )office\.com|microsoft365\.com|officeapps\.live|office365\.com/i.test(site);
    if (!onM365) {
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    // DEBUG trace: show EXACTLY what the LLM decided each step (op + key arg), so a single
    // test reveals the whole plan and a wrong turn is pinpointed instead of guessed.
    if (action && action.op) {
      const arg = action.query || action.instruction || action.text || action.fromText || action.url || action.dir || (action.ref != null ? "ref " + action.ref : "");
      addStep("· op=" + action.op + (arg ? " → " + String(arg).slice(0, 60) : ""));
    }
    ...
    // Search the user's WHOLE Microsoft 365 (mail/files/Teams) — not just this page. Result
    // goes into notes so the LLM can then act (open a found mail, draft a reply, …).
    if (action.op === "search_m365" || action.op === "search_mail" || action.op === "search_files" || action.op === "find_email") {
      const q = action.query || action.text || goal;
      addStep("🔎 M365 paieška: " + q);
      const res = await searchM365ForAgent(q);
      notes.push(res);
      continue;
    }
...
// ---- Microsoft 365 "search my documents" flow -------------------------------------
// When the active tab is a Microsoft product (Outlook / SharePoint / Teams), a question is
// answered from BOTH the visible page AND a delegated Graph search over the user's own
// M365 (via gp-m365) — no extra button. Detection is by the active tab URL.
// Detect ANY Microsoft 365 app (not just Outlook). Returns a search SCOPE:
//   outlook → mail · teams → chats/files · sharepoint → files · m365 → everything.
// Covers classic domains AND the new *.cloud.microsoft ones, plus Word/Excel/PPT/OneNote
// online, OneDrive, and the office.com hub. Anything M365 → the question searches the
// user's whole M365; only the scope narrows the search for speed/relevance.
async function detectMsProduct() {
  try {
    const [tab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true });
    const u = ((tab && tab.url) || "").toLowerCase();
    console.log("[GP] detectMsProduct url=", u);
    if (/outlook\.(office|office365|live)\.com|\/\/outlook\.com|outlook\.cloud\.microsoft/.test(u)) return "outlook";
    if (/teams\.(microsoft\.com|live\.com|cloud\.microsoft)/.test(u)) return "teams";
    if (/[\w-]+\.sharepoint\.com|onedrive\.live\.com|sharepoint\.cloud\.microsoft|onedrive\.cloud\.microsoft/.test(u)) return "sharepoint";
    // Word/Excel/PowerPoint/OneNote online, the office.com hub, and any other M365 surface
    // → generic "m365" scope (search mail + files together).
    if (/(word|excel|powerpoint|onenote|m365|office|create)\.cloud\.microsoft|\.cloud\.microsoft|(www\.)?office\.com|microsoft365\.com|officeapps\.live\.com|\.office365\.com/.test(u)) return "m365";
  } catch (e) {}
  return null;
}

// Delegated Graph token via the OWUI-hosted MSAL page (/_gp/msauth.html), returned by
// postMessage. Silent first (hidden iframe); on interaction_required, a one-time popup.
// Cached ~50 min. NEVER handled by gp-m365 raw — the token stays client-side and is only
// forwarded to gp-m365, which searches AS the user.
// The email the conversation is currently ABOUT (set when gp-m365 answers about a message),
// so "atsakyk" replies to THAT one — opening it first — even if a different mail is on screen.
let _focusEmail = null;
let _gphTok = null, _gphExp = 0;
// Resolves {token} or {error}. Opens the OWUI-hosted MSAL page as a POPUP (top-level
// context) — a hidden iframe inside the side panel can't do silent SSO (the nested AAD
// iframe's 3rd-party cookies are blocked), but a top-level popup can (verified). MSAL is
// silent when an AAD session exists, so the popup flashes briefly and self-closes; token
// is cached ~50 min. Error strings are surfaced so a failure is never a silent dead end.
function getGraphToken() {
  return new Promise((resolve) => {
    if (_gphTok && Date.now() < _gphExp) return resolve({ token: _gphTok });
    let origin; try { origin = new URL(BASE).origin; } catch (e) { return resolve({ error: "bad BASE: " + BASE }); }
    const url = BASE + "/_gp/msauth.html?interactive=1";
    let done = false, win = null, timer = null;
    const cleanup = () => { window.removeEventListener("message", onMsg); if (win) try { win.close(); } catch (e) {} clearTimeout(timer); };
    const finish = (v) => { if (done) return; done = true; cleanup(); resolve(v); };
    const onMsg = (ev) => {
      if (ev.origin !== origin) return;                    // only trust the OWUI origin
      const d = ev.data || {};
      if (d.gpToken) { _gphTok = d.gpToken; _gphExp = Date.now() + 50 * 60 * 1000; finish({ token: d.gpToken }); }
      else if (d.gpError) finish({ error: "msauth: " + d.gpError + (d.gpDetail ? " — " + d.gpDetail : "") });
    };
    window.addEventListener("message", onMsg);
    win = window.open(url, "gpmsauth", "width=480,height=600");
    if (!win) return finish({ error: "popup užblokuotas — leisk iššokančius langus šiam puslapiui" });
    timer = setTimeout(() => finish({ error: "baigėsi laikas laukiant Microsoft prisijungimo" }), 90000);
  });
}

// Agent TOOL: search the user's whole Microsoft 365 (mail + files + Teams) via gp-m365 and
// return a text result for the agent's notes, so the LLM can then act (open a mail, reply…).
async function searchM365ForAgent(query) {
  const product = await detectMsProduct();
  if (!product) return "search_m365 negalimas: ne Microsoft 365 programa.";
  const tok = await getGraphToken();
  if (!tok || !tok.token) return "search_m365 nepavyko (prieiga): " + ((tok && tok.error) || "?");
  try {
    const r = await api("/_gp/m365", { method: "POST",
      body: JSON.stringify({ question: query, graph_token: tok.token, product, history: CONVO.slice(-6) }) });
    if (!r.ok) return "search_m365 HTTP " + r.status;
    const d = await r.json();
    const list = (d.sources || []).map((s) => "• " + (s.title || s.source)).join("\n");
    return "M365 PAIEŠKA („" + query + "“):\n" + (d.answer || "") + (list ? "\nRasti įrašai (gali atidaryti pagal temą):\n" + list : "");
  } catch (e) { return "search_m365 klaida: " + (e.message || e); }
}

async function askM365(goal, model, product) {
  setRunning(true);
  const think = addMsg("assistant", "🔎 Ieškau jūsų " + product + " duomenyse…");
  try {
    let pageText = "";
    try { const ctx = await captureContext(); pageText = ((ctx && ctx.text) || "").slice(0, 12000); } catch (e) {}
    const tok = await getGraphToken();
    if (!tok || !tok.token) { think.className = "msg err"; think.innerHTML = esc("Nepavyko gauti M365 prieigos — " + ((tok && tok.error) || "nežinoma klaida")); return; }
    const token = tok.token;
    const r = await api("/_gp/m365", { method: "POST",
      body: JSON.stringify({ question: goal, graph_token: token, page_context: pageText, product,
        history: CONVO.slice(-6) }) });
    if (r.status === 401 || r.status === 403) { think.className = "msg err"; think.innerHTML = esc("Neprisijungta prie OWUI."); return; }
    if (!r.ok) throw new Error("HTTP " + r.status + " " + (await r.text()).slice(0, 160));
    const d = await r.json();
    think.innerHTML = esc(d.answer || "(tuščias atsakymas)");
    // Remember the email this answer is about → "atsakyk" opens THAT one, not what's on screen.
    const msgSrc = (d.sources || []).find((s) => s.source === "message" && s.title);
    if (msgSrc) _focusEmail = { subject: String(msgSrc.title).split("—")[0].trim(), web_url: msgSrc.web_url || "" };
    if (Array.isArray(d.sources) && d.sources.length) {
      const src = document.createElement("div");
      src.style.cssText = "font-size:12px;opacity:.7;margin-top:4px";
      src.innerHTML = "Šaltiniai: " + d.sources.map((s) =>
        s.web_url ? `<a href="${esc(s.web_url)}" target="_blank">${esc(s.title || s.source)}</a>` : esc(s.title || s.source)).join(" · ");
      think.appendChild(src);
    }
    remember(goal, d.answer || "");
  } catch (e) {
    think.className = "msg err";
    think.innerHTML = esc("Klaida: " + (e.message || e));
  } finally {
    setRunning(false);
  }
}
...
async function send() {
  ...
  addMsg("user", goal);
  // Microsoft product tab: split by intent.
  //  * ACTION (reply/write/send/delete/…) → the AGENT (runAgent) — it opens the mail, drafts
  //    into the reply, and you send/confirm. gp-m365 is server-side and CANNOT act on the page.
  //  * QUESTION (find/do-I-have/summarise/…) → gp-m365 (searches the whole M365).
  // The AGENT (LLM brain) handles everything — on Microsoft 365 it has the search_m365 tool
  // to look across the whole account AND the page ops (click/draft_reply/forward/…) to act,
  // so it understands and plans ANY request (incl. compound "find X and reply") itself. No
  // brittle keyword routing.
  await runAgent(goal, model);
}

// A clear REPLY request → draft deterministically (don't depend on the agent LLM choosing
// the op). Opens the reply form, composes from the open email, fills the body, stops.
function isReplyIntent(g) {
  return /(^|\s)(atsakyk|atsakymą|atsakyti|atsak|reply|respond|atrašyk|atrasyk)/i.test(g || "");
}

// Standalone reply drafter — the same proven flow as the agent's draft_reply op, callable
// directly so "atsakyk …" ALWAYS opens + fills the reply (no model op-choice in the loop).
async function draftReply(goal, model, which) {
  setRunning(true);
  try {
    // If the conversation is about a specific email (from a prior search), OPEN it first so
    // we reply to THAT one — not whatever happens to be on screen.
    if (_focusEmail && _focusEmail.subject) {
      addMsg("assistant", "atidarau laišką: „" + _focusEmail.subject + "“");
      const oc = await clickTextInPage(_focusEmail.subject);
      if (oc && oc.ok) { const [tb] = await chrome.tabs.query({ active: true, lastFocusedWindow: true }); if (tb && tb.id) await waitSettle(tb.id, 1500); }
      else addMsg("assistant", "⚠ neradau to laiško sąraše — atsakau į atidarytą.");
      _focusEmail = null;   // one-shot: consume it so a later plain "atsakyk" uses the open mail
    }
    let ctx; try { ctx = await captureContext(); } catch (e) { ctx = { text: "" }; }
    const emailCtx = (ctx.text || "").slice(0, 6000);
    const w = (which || "reply").toLowerCase();
    const openLabels = /all|visiem|всем/.test(w)
      ? ["Ответить всем", "Reply all", "Reply to all", "Atsakyti visiems", "Allen antworten"]
      : ["Ответить", "Atsakyti", "Reply", "Antworten", "Répondre"];
    addMsg("assistant", "atidarau atsakymo langą…");
    const orr = await clickExactInPage(openLabels);
    if (!(orr && orr.ok)) { addMsg("error", "Neradau „Atsakyti“/„Reply“ mygtuko — atidaryk laišką, į kurį atsakai."); return; }
    { const [tb] = await chrome.tabs.query({ active: true, lastFocusedWindow: true }); if (tb && tb.id) await waitSettle(tb.id, 1800); }
    const composePrompt =
      "Parašyk MANDAGŲ atsakymo el. laišką TA PAČIA kalba kaip originalas. Originalus laiškas:\n«" +
      emailCtx + "»\n\nKO reikia atsakyti / užduotis: «" + goal + "»\n\n" +
      "Grąžink TIK galutinį laiško tekstą: kreipimasis, turinys, ir pabaigoje mandagus atsisveikinimas (pvz. „Pagarbiai,“) BE vardo ir BE el. pašto — parašą užbaigs žmogus. " +
      "NEKARTOK užduoties teksto, NERAŠYK pastabų/antraščių/temos/kabučių. NEPRASIMANYK vardų, el. paštų, telefonų. GP_xxxx/[PERSON] cituok kaip yra.";
    let out;
    try { out = await askLLM(model, composePrompt, { isolate: true, noSkill: true }); }
    catch (e) { if (e && e.name === "AbortError") { addMsg("assistant", "⏹ Sustabdyta."); return; } addMsg("error", "Nepavyko sukurti juodraščio: " + (e.message || e)); return; }
    if (out.authFail) return loginHint();
    const draft = unwrapAnswer(out.raw);
    if (!draft) { addMsg("error", "Tuščias juodraštis."); return; }
    const res = await fillComposeInPage(draft);
...
// Imperative action verbs → route to the page agent, not the search. LT + EN.
function isActionIntent(g) {
  return /(^|\s)(atsakyk|atsakymą|atsakyti|paraš|parašyk|parasyk|rašyk|rasyk|siųsk|siusk|išsiųsk|issiusk|persiųsk|persiusk|ištrink|istrink|trink|pašalink|pasalink|sukurk|perraš|perrasyk|redaguok|pakeisk|užpildyk|uzpildyk|pažymėk|pazymek|archyvuok|priseg|perkelk|reply|respond|write|draft|compose|send|forward|delete|remove|fill|edit)/i.test(g || "");
}
