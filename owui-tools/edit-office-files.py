...
version: 4.0.5
requirements: openpyxl, python-docx, python-pptx, xlrd, odfpy, docx-revisions, lxml, PyMuPDF, Pillow, pytesseract, qrcode
"""
...
def _validate_outbound_url(url: str) -> Optional[str]:
    ...
    """
    # GuardPrompt hardening: outbound network is DISABLED by default. All three
    # URL-fetching functions (import_from_url, import_from_api, webhook_trigger) call
    # this first, so rejecting here neutralises every prompt-injection exfiltration
    # path in one place. Set OFFICE_TOOL_ALLOW_EGRESS=true to re-enable (then the
    # SSRF checks below still apply). On-prem + anonymized -> keep it off.
    if os.getenv("OFFICE_TOOL_ALLOW_EGRESS", "false").strip().lower() not in ("1", "true", "yes", "on"):
        return "Outbound network is disabled in this deployment (OFFICE_TOOL_ALLOW_EGRESS=false)."
...
class Tools:
    ...
    async def upload_to_drive(self, file_id: str, folder_id: str = "root") -> str:
        ...
        # GuardPrompt hardening: Google Drive egress is disabled (on-prem, anonymized —
        # never send a document to a third-party cloud). Gated behind the same flag.
        if os.getenv("OFFICE_TOOL_ALLOW_EGRESS", "false").strip().lower() not in ("1", "true", "yes", "on"):
            return json.dumps({"error": "Google Drive upload is disabled in this deployment (OFFICE_TOOL_ALLOW_EGRESS=false)."})
        try:
    ...
    async def bulk_folder_ops(self, operation: str = "list", pattern: str = "*", __user__=None, __request__=None) -> str:
        ...
        """
        # GuardPrompt hardening: enumerating the whole uploads folder exposes EVERY
        # user's files to the LLM (privacy leak) and derails simple single-file edits.
        # Disabled by default — the tool should act only on the file the user attached.
        # Set OFFICE_TOOL_ALLOW_FOLDER_OPS=true to re-enable.
        if os.getenv("OFFICE_TOOL_ALLOW_FOLDER_OPS", "false").strip().lower() not in ("1", "true", "yes", "on"):
            return json.dumps({"error": "Aplankų operacijos išjungtos. Dirbk su prisegtu failu (nurodyk file_id)."})
    ...
    async def file_search(self, query: str, file_type: str = "all") -> str:
        ...
        # GuardPrompt hardening: full-uploads search exposes other users' files to the LLM.
        # Disabled by default (see OFFICE_TOOL_ALLOW_FOLDER_OPS). Work with the attached file.
        if os.getenv("OFFICE_TOOL_ALLOW_FOLDER_OPS", "false").strip().lower() not in ("1", "true", "yes", "on"):
            return json.dumps({"error": "Failų paieška visame aplanke išjungta. Dirbk su prisegtu failu."})
