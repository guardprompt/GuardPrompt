...
# --- Office extraction via MarkItDown ---------------------------------------
# docling is tuned for PDFs (OCR, layout, TableFormer) but weak on Office: it DROPS
# Excel sheet names and PowerPoint speaker notes, which are exactly the context a KB
# answer needs. MarkItDown (pure Office-XML parsing, no ML/GPU) keeps them:
#   xlsx -> "## <sheet name>" per sheet;  pptx -> "<!-- Slide N -->" + "### Notes:".
# So Office (docx/xlsx/pptx) is routed to MarkItDown; PDFs/images stay on docling
# (its strength). On any MarkItDown failure/empty output the caller falls back to
# docling, so this can only add coverage, never remove it. Toggle per env.
OFFICE_MARKITDOWN = os.getenv("OFFICE_MARKITDOWN", "true").strip().lower() in ("1", "true", "yes", "on")
OFFICE_MD_EXTS = {e.strip().lower() for e in os.getenv("OFFICE_MARKITDOWN_EXTS", ".docx,.xlsx,.pptx").split(",") if e.strip()}
print("Office MarkItDown:", OFFICE_MARKITDOWN, sorted(OFFICE_MD_EXTS))
...
def _office_markitdown(path: str):
    """Office file -> markdown via MarkItDown. Returns None on failure/empty so the
    caller falls back to docling (never a hard failure)."""
    try:
        md = _markitdown().convert(path).text_content or ""
    except Exception as e:
        print(f"[markitdown] failed ({e!r}) -> docling fallback", flush=True)
        return None
    if not md.strip():
        return None
    # A merged title row above a table becomes "| Title | Unnamed: 1 | Unnamed: 2 |"
    # (pandas placeholders for the spanned empty header cells). Strip the noise.
    md = re.sub(r"Unnamed:\s*\d+", "", md)
    return md
...
def create_pdf_converter(do_ocr: bool, do_picture_description: bool, prompt: str | None):
    ...
    pipeline = PdfPipelineOptions(
        ...
        # Page images ON: docling's markdown serializer, when a PictureItem has no own
        # raster (small/vector/EMF figures, or generation skipped), FALLS BACK to cropping
        # the picture out of the PAGE image. With page images OFF there is no fallback, so
        # such a picture serialized as "<!-- Image not available. Please use
        # generate_picture_images=True -->" and the picture-description step got nothing —
        # the reported regression. The page render is also what the crop is taken from.
...
async def convert(file: UploadFile = File(...)):
    ...
    def run_once(flag):
        ...
        # Office (docx/xlsx/pptx) -> MarkItDown (keeps xlsx sheet names + pptx notes
        # docling drops). None on failure/empty -> fall through to docling below.
        md = None
        if OFFICE_MARKITDOWN and ext in OFFICE_MD_EXTS:
            md = _office_markitdown(str(path))
        ...
        # Safety net: never let docling's raw "Image not available / generate_picture_images"
        # placeholder comment reach the KB (a vector/EMF figure in a Word doc has no raster and
        # no page-image fallback, so the serializer still emits it). Drop the comment silently —
        # the surrounding text stays; a real raster is embedded+described above/below.
        md = re.sub(r'[ \t]*<!--[^>]*Image not available[^>]*-->[ \t]*\n?', '', md)
        md = md.strip()
        md = re.sub(r'(?m)^\s*-{3,}\s*$', '', md)
        md = re.sub(r'[ \t]{2,}', ' ', md)
        md = re.sub(r'\n{3,}', '\n\n', md)
