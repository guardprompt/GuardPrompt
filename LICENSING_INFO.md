> 🌐 **Language / Kalba:** **English** · [Lietuvių](LICENSING_INFO.lt.md)
...
- GuardPrompt Claude proxy (developer tooling) and the Zabbix monitoring stack  
...
These services are billed separately by their providers. The Claude proxy ships
with GuardPrompt, but the **Anthropic subscription (Pro/Team) or API key it
relays is the customer's own cost** — GuardPrompt neither provides nor bills it.
